...
def track_and_visualize(video_sequence, initial_rectangle, max_iterations, convergence_threshold):
    """
    Tracks the car in the sequence and visualizes the tracking at specific frames.
    ...
    for frame_index in range(total_frames - 1):
        ...
        # Visualize tracking at frames 1, 100, 200, 300, 400
...
def visualize_tracking(frame, rectangle, frame_number):
    ...
    plt.savefig(f'../results/carseq{frame_number}.png', bbox_inches='tight')

parser = argparse.ArgumentParser()
parser.add_argument('--num_iters', type=int, default=1e4, help='number of iterations of Lucas-Kanade')
parser.add_argument('--threshold', type=float, default=1e-6, help='dp threshold of Lucas-Kanade for terminating optimization')
...
rects = track_and_visualize(seq, rect, num_iters, threshold)
